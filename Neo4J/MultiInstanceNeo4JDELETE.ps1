...
{	# Define server specific parameters and Neo4J credentials
	...
	$NeoCommand = Invoke-DbaQuery -SqlInstance $SourceServer -Database $DataStoreDB -CommandType StoredProcedure -Query "maintenance.uspGetNeo4jObjectsToDelete" -EnableException # Get Neo Delete Commands to execute
	cd $cypherShellLoc # change dir to cypher shell location
    foreach ($c in $NeoCommand)
	{
		$cypherCmd = $c[0]
